...
- **NO.001**（rules_only）：128 万元软件服务合同 → 3 项高风险、通过率 65.4%
- **NO.002**（rules_agent + DeepSeek）：200 万元智能硬件合同 → LLM 复核报告写回、通过率 69.2%
- **NO.003**（rules_agent，真实底稿）：依据政府采购网真实中标公告还原的医院绩效系统合同（57.95 万元，乙方为真实主体上海东旦软件开发有限公司）→ 3 项高风险、通过率 69.2%、**真实联网尽调**（0 失信 0 处罚、2 条原告身份涉诉）写回主体画像
- **Token A/B 实测**：同一合同、同一模型，纯 LLM 两轮 18327 tokens vs 混合架构单轮 13705 tokens，**节省 25.2%**（API usage 实计，脚本 `scripts/token_ab_test.py`）
- **3 份主体画像**写回飞书 wiki（审核历史 + 联网尽调结论）
...
| LLM 复核 | `src/local_crewai_demo/crew.py`（CrewAI 2 Agent：审核 + 汇报大纲） | 仅此层耗 token，A/B 实测节省 25.2% |
...
1. **重录演示视频**（今晚，最高优先级）：分镜脚本见 `docs/DEMO_VIDEO_SCRIPT.md`，主线用 NO.003 真实底稿合同跑全链路。
2. **小浣熊配置 Skill**（约 10 分钟）：技能 → 快速新建 Skill → 粘贴 `skills/contract-review-loop/SKILL.md`；对话测试「使用 contract-review-loop 技能跑一次合同审查闭环」；把定时任务 Prompt 换成 `docs/XIAOHUANXIONG_MCP_SETUP.md`「一·五」的版本。
3. **补飞书 UI 截图**（路演证据）：wiki 三合同 + 报告、多维表格 NO.001-003、三份主体画像（重点拍上海东旦的真实尽调节）。
4. **（可选）恢复 HF Space** 拿稳定访问链接（`docs/DEPLOY_NO_CARD.md`）。
5. **彩排讲述顺序（对齐评委五段式）**：① 场景洞察（痛点+亮点）→ ② 技术方案（三层 skill + 六能力编排）→ ③ 当前成果（3 单闭环实测 + 商业账）→ ④ 播放录屏 + 现场开飞书链接 → ⑤ 远期规划 + 团队。

### 路演答问口径（已核实）

- **「数字有没有夸大？」**：3 高风险 / 65.4% 通过率为 128 万元合同实测；token 节省 25.2% 为 A/B 双架构真实调用实测（`scripts/token_ab_test.py`，结果 `outputs/token-ab-result.json`）；4h→3min 为估算（PPT 已标注）；主体画像与尽调已真实实现并写回飞书。
- **「合同和主体是真的吗？」**：NO.003 依据中国政府采购网真实中标公告还原（明光市中医院绩效系统项目，成交 57.95 万元），乙方上海东旦软件开发有限公司为真实主体，尽调结果（0 失信 0 处罚、2 条原告身份涉诉）可当场联网复查。
